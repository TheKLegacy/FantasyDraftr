...
export const allocateBoards = (boards: BoardPayload[]): Board[] => boards.map(b => ({...b, Players: allocatePlayers(b.Players)}) as Board);

const allocatePlayers = (players: string[]): Player[] => {
    return players.map(p => playerObjects.find(o => o.player_id === p)!);
}