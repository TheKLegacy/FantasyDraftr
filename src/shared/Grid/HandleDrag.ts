...
export function handleDrag<T extends { [key: string]: any }>(
    ...
    const newIndex = updatedItems.findIndex((item) => item[idKey] === itemsInView[event.overIndex][idKey]);

    // Remove dragged items from the original array
    draggedData.forEach((item) => {
        updatedItems = updatedItems.filter((existingItem) => existingItem[idKey] !== item[idKey]);
    });
