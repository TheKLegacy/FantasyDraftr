...
export const Actions = (props: {data: Player}) => {
    ...
    const movePlayer = (offset: number): void => {
        ...
        const currentIndex = updatedData.findIndex(p => p.player_id === player.player_id);
    ...
    return (
        <div>
            <Tooltip title="Down 10">
                <IconButton onClick={() => movePlayer(-10)}><ExpandMoreIcon /></IconButton>
            </Tooltip>
            <Tooltip title="Down 20">
                <IconButton onClick={() => movePlayer(-20)}><KeyboardDoubleArrowDownIcon /></IconButton>
            </Tooltip>
            <Tooltip title="Up 10">
                <IconButton onClick={() => movePlayer(10)}><KeyboardArrowUpIcon /></IconButton>
            </Tooltip>
            <Tooltip title="Up 20">
                <IconButton onClick={() => movePlayer(20)}><KeyboardDoubleArrowUpIcon /></IconButton>
            </Tooltip>
        </div>
    );
};